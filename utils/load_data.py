...
def load_gaussian_data(args, **kwargs):
    # set args
    args.input_size = [1]
    args.input_type = 'continuous'
    args.dynamic_binarization = False
    ...
    if args.num_gaussians == 2:
        size = int ((train_size + test_size + val_size) / args.num_gaussians)
        m1 = normal.Normal (args.gaussian_mean1, args.gaussian_var1)
        x1 = m1.sample ((size, 1))
        m2 = normal.Normal (args.gaussian_mean2, args.gaussian_var2)
        x2 = m2.sample ((size, 1))
        x = torch.cat([x1[:, 0], x2[:, 0]])

    else:
        raise Exception('Wrong number of the gaussians!')
    ...
    test_idx = idx[train_size:len(idx)-test_size]
    val_idx = idx[len(idx)-val_size:]

    x_train = x.view(-1)[train_idx].view(train_idx.size())
    x_test = x.view(-1)[test_idx].view(test_idx.size())
    x_val = x.view(-1)[val_idx].view(val_idx.size())
    ...
    # pytorch data loader
    train = data_utils.TensorDataset(x_train, torch.from_numpy(y_train))
    train_loader = data_utils.DataLoader(train, batch_size=args.batch_size, shuffle=True, **kwargs)

    validation = data_utils.TensorDataset(x_val.float(), torch.from_numpy(y_val))
    val_loader = data_utils.DataLoader(validation, batch_size=args.test_batch_size, shuffle=False, **kwargs)

    test = data_utils.TensorDataset(x_test, torch.from_numpy(y_test))
...
#        train_loader, val_loader, test_loader, args = load_freyfaces(args, **kwargs)
    ...
    elif args.dataset_name == 'gaussian_data':
        train_loader, val_loader, test_loader, args = load_cifar10 (args, **kwargs)
