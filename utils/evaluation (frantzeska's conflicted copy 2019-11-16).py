...
def evaluate_vae(args, model, train_loader, data_loader, epoch, dir, mode):
    ...
    if mode == 'test':
        ...
        if args.prior == 'conditional' :
            ...
            z_q_mean, z_q_logvar, z_q_discr = model.encoder(test_data)
            ...
            num_correct = torch.sum(test_target == indices, dim=-1)
            accuracy = (float(num_correct) / len(data_loader)) * 100
            print('accuracy')
            print(accuracy)
            ...
            for i in range(25):
                sample_rand_marginal, _, _, _ = model.generate_cont_x_marginal (average_q_c_x)
            ...
            for i in range(25):
                sample_rand, samples_rand_var, prior_means, prior_vars = model.generate_cont_x ()
        ...
        if args.prior == 'joint' :
            ...
            samples_rand_cz_marginal =[]

            for i in range(25):
                zc_sample_means_marginal, _, _, _ = model.generate_cont_x_marginal (average_q_c_x)
                samples_rand_cz_marginal.append(zc_sample_means_marginal)
            samples_means_marginal = torch.cat(samples_rand_cz_marginal)
            #
            plot_images(args, samples_means_marginal.data.cpu().numpy(), dir + 'figures/', 'generations_marginal', size_x=5, size_y=5) #size_x=5, size_y=5)

            samples_rand_cz =[]

            for i in range(25):
                zc_sample_means, _, _, _ = model.generate_x_joint ()
            ...
            for j in range(args.disc_size):
                samples_rand_specific =[]
                for i in range(25):
                    sample_rand_specific = model.generate_specific_x_cat_joint (j)
                    samples_rand_specific.append(sample_rand_specific)
                sample_rand_specific = torch.cat(samples_rand_specific)
        ...
        #         for i in range(10):
        #             for j in range(len(test_target)):
        #                 if test_target[j].data.cpu().numpy()%10 == i:
        #                     samples_rand = model.traversal(test_data[j])
       ...
       # _, _, _, _, z_q_discr_r, _, _, z_q_discr = model.forward(test_data)
       # q_c_x = F.softmax(z_q_discr, dim=-1)
